...
GETCMD    EQU    1
RORSW    EQU    1
ROMSW    EQU    1            ;TELLS IF THIS IS ON ROM.
CLMWID    EQU    14
LONGI    EQU    1            ;LONG INITIALIZATION SWITCH.
STKEND    EQU    511
BUFPAG    EQU    0
LINLEN    EQU    72            ;TERMINAL LINE LENGTH.
BUFLEN    EQU    72            ;INPUT BUFFER SIZE.
ROMLOC    EQU     $2000            ;ADDRESS OF START OF PURE SEGMENT.
KIMROM    EQU    1
IFE    ROMSW,<KIMROM    EQU    0>
IFN    REALIO-1,<KIMROM    EQU    0>
IFN    ROMSW,<
RAMLOC    EQU     $4000            ;USED ONLY IF ROMSW=1
IFE    REALIO,<ROMLOC= $2000 ;START AT 8K.
RAMLOC    EQU    $300>>
IFE    REALIO-3,<
DISKO    EQU    1
RAMLOC    EQU    $400
...
CQOPEN    EQU    $FFC0
CQCLOS    EQU    $FFC3
CQOIN    EQU     $FFC6        ;OPEN CHANNEL FOR INPUT
CQOOUT    EQU    $FFC9        ;FILL FOR COMMO.
CQCCHN    EQU    $FFCC
CQINCH    EQU    $FFCF        ;INCHR'S CALL TO GET A CHARACTER
OUTCH    EQU     $FFD2
CQLOAD    EQU    $FFD5
CQSAVE    EQU    $FFD8
CQVERF    EQU    $FFDB
CQSYS    EQU     $FFDE
ISCNTC    EQU    $FFE1
CZGETL    EQU    $FFE4        ;CALL POINT FOR "GET"
CQCALL    EQU    $FFE7        ;CLOSE ALL CHANNELS
CQTIMR    EQU    $8D
BUFPAG    EQU    2
BUF    EQU    256*BUFPAG
STKEND    EQU    507
...
GETCMD    EQU    1
CLMWID    EQU    10
PI    EQU    255                ;VALUE OF PI CHARACTER FOR COMMODORE.
ROMSW    EQU    1
RORSW    EQU    1
TRMPOS    EQU    $C6>
IFE    REALIO-1,<GETCMD    EQU    1
DISKO    EQU    1
OUTCH    EQU    $1EA0            ;1EA0
ROMLOC    EQU    $2000
RORSW    EQU    0
...
DEFINE    INCW(R),<
    ...
    INC    R+1
%Q:>
DEFINE    SKIP1,    <XWD $200,$24>    ;BIT ZERO PAGE TRICK.
DEFINE    SKIP2,    <XWD $200,$2C>    ;BIT ABS TRICK.
IF1,<
...
*
* --- GENERAL RAM ---:
CHARAC: DS    1        ;A DELIMITING CHARACTER.
INTEGR    EQU     CHARAC            ;A ONE-BYTE INTEGER FROM "QINT".
ENDCHR: DS    1        ;THE OTHER DELIMITING CHARACTER.
COUNT:    DS    1        ;A GENERAL COUNTER.
...
SUBFLG: DS    1        ;FLAG WHETHER SUB'D VARIABLE ALLOWED.
...
INPFLG: DS    1        ;FLAGS WHETHER WE ARE DOING "INPUT"
                ;OR "READ".
TANSGN: DS    1        ;USED IN DETERMINING SIGN OF TANGENT.
IFN    REALIO,<
CNTWFL: DS    1>        ;SUPPRESS OUTPUT FLAG.
...
TEMPPT: DS    1        ;POINTER AT FIRST FREE TEMP DESCRIPTOR.
                ;INITIALIZED TO POINT TO TEMPST.
LASTPT: DS    2        ;POINTER TO LAST-USED STRING TEMPORARY.
TEMPST: DS    STRSIZ*NUMTMP    ;STORAGE FOR NUMTMP TEMP DESCRIPTORS.
INDEX1: DS    2        ;INDEXES.
INDEX    EQU        INDEX1
INDEX2: DS    2
RESHO:    DS    1        ;RESULT OF MULTIPLIER AND DIVIDER.
IFN    ADDPRC,<
RESMOH: DS    1>        ;ONE MORE BYTE.
RESMO:    DS    1
RESLO:    DS    1
ADDEND    EQU     RESMO            ;TEMPORARY USED BY "UMULT".
    DC    I1'0'        ;OVERFLOW FOR RES.
...
FRESPC: DS    2        ;POINTER TO NEW STRING.
...
INPPTR: DS    2        ;THIS REMEMBERS WHERE INPUT IS COMING FROM.

* --- STUFF USED IN EVALUATIONS ---:
VARNAM: DS    2        ;VARIABLE'S NAME IS STORED HERE.
VARPNT: DS    2        ;POINTER TO VARIABLE IN MEMORY.
FDECPT    EQU     VARPNT            ;POINTER INTO POWER OF TENS OF "FOUT".
FORPNT: DS    2        ;A VARIABLE'S POINTER FOR "FOR" LOOPS
                ;AND "LET" STATEMENTS.
...
OPPTR:    DS    2        ;POINTER TO CURRENT OP'S ENTRY IN "OPTAB".
VARTXT    EQU     OPPTR            ;POINTER INTO LIST OF VARIABLES.
OPMASK: DS    1        ;MASK CREATED BY CURRENT OPERATOR.
DOMASK    EQU    TANSGN            ;MASK IN USE BY RELATION OPERATIONS.
DEFPNT: DS    2        ;POINTER USED IN FUNCTION DEFINITION.
GRBPNT    EQU     DEFPNT            ;ANOTHER USED IN GARBAGE COLLECTION.
DSCPNT: DS    2        ;POINTER TO A STRING DESCRIPTOR.
IFN    ADDPRC,<DS    1>    ;FOR TEMPF3.
FOUR6:    DB    STRSIZ        ;VARIABLE CONSTANT USED BY GARB COLLECT.
...
HIGHDS: DS    2        ;DESINATION OF HIGHEST ELEMENT IN BLT.
HIGHTR: DS    2        ;SOURCE OF HIGHEST ELEMENT TO MOVE.
TEMPF2:
IFN    ADDPRC,<0>        ;FOR TEMPF2S EXTRA BYTE.
LOWDS:    DS    2        ;LOCATION OF LAST BYTE TRANSFERRED INTO.
LOWTR:    DS    2        ;LAST THING TO MOVE IN BLT.
...
FAC:
FACEXP: DC    I1'0'
FACHO:    DC    I1'0'        ;MOST SIGNIFICANT BYTE OF MANTISSA.
IFN    ADDPRC,<
FACMOH: DC    I1'0'>        ;ONE MORE.
FACMO:    DC    I1'0'        ;MIDDLE ORDER OF MANTISSA.
FACLO:    DC    I1'0'        ;LEAST SIG BYTE OF MANTISSA.
FACSGN: DC    I1'0'        ;SIGN OF FAC (0 OR -1) WHEN UNPACKED.
SGNFLG: DC    I1'0'        ;SIGN OF FAC IS PRESERVED BERE BY "FIN".
...
BITS:    DC    I1'0'        ;SOMETHING FOR "SHIFTR" TO USE.

* --- THE FLOATING ARGUMENT (UNPACKED) ---:
ARGEXP: DC    I1'0'
ARGHO:    DC    I1'0'
IFN    ADDPRC,<ARGMOH: DC    I1'0'>
ARGMO:    DC    I1'0'
ARGLO:    DC    I1'0'
ARGSGN: DC    I1'0'

ARISGN: DC    I1'0'        ;A SIGN REFLECTING THE RESULT.
FACOV:    DC    I1'0'        ;OVERFLOW BYTE OF THE FAC.
STRNG1    EQU     ARISGN            ;POINTER TO A STRING OR DESCRIPTOR.

FBUFPT: DS    2        ;POINTER INTO FBUFFR USED BY FOUT.
...
CHRGET: INC    CHRGET+7    ;INCREMENT THE WHOLE TXTPTR.
    BNE    CHRGOT
    INC    CHRGET+8
CHRGOT: LDA    $EA60        ;A LOAD WITH AN EXT ADDR.
TXTPTR    EQU     CHRGOT+1
    CMP #" "        ;SKIP SPACES.
    BEQ    CHRGET
QNUM:    CMP #":"        ;IS IT A ":"?
...
RNDX:    DC    I1'128'        ;LOADED OR FROM ROM.
    DC    I1'79'        ;THE INITIAL RANDOM NUMBER.
    DC    I1'199'
    DC    I1'82'
IFN    ADDPRC,<89>        ;ONE MORE BYTE.

ORG    255            ;PAGE 1 STUFF COMING UP.
LOFBUF: DS    1        ;THE LOW FAC BUFFER. COPYABLE.
* ---  PAGE ZERO/ONE BOUNDARY ---.
                ;MUST HAVE 13 CONTIGUOUS BYTES.
FBUFFR: DS    3*ADDPRC+13    ;BUFFER FOR "FOUT".
...
MINUTK    EQU    Q
    ...
    DCI"AND"
    DCI"OR"
    DC    I1'190'        ;A GREATER THAN SIGN
Q    EQU    Q+1
GREATK    EQU    Q
    DCI"="
EQULTK    EQU    Q
    DC    I1'188'
Q    EQU    Q+1            ;A LESS THAN SIGN
LESSTK    EQU    Q
*
...
LASNUM    EQU    Q            ;NUMBER OF LAST FUNCTION
    ...
    DCI"GO"
GOTK    EQU    Q
    DC    I1'0'        ;MARKS END OF RESERVED WORD LIST
...
ERROD    EQU    Q        ;OUT OF DATA.
    DCE"FC"
ERRFC    EQU    Q        ;ILLEGAL QUANTITY.
    DCE"OV"
ERROV    EQU    Q        ;OVERFLOW.
    DCE"OM"
ERROM    EQU    Q        ;OUT OF MEMORY.
    DCE"US"
ERRUS    EQU    Q        ;UNDEFINED STATEMENT.
    DCE"BS"
ERRBS    EQU    Q        ;BAD SUBSCRIPT.
    DCE"DD"
ERRDD    EQU    Q        ;REDIMENSIONED ARRAY.
    DCE"/0"
ERRDV0    EQU    Q        ;DIVISION BY ZERO.
    DCE"ID"
...
ERRCN    EQU    Q        ;CAN'T CONTINUE.
    DCE"UF"
ERRUF    EQU    Q>        ;UNDEFINED FUNCTION.
...
GETSTK: ASL    A,        ;MULT [A] BY 2. NB, CLEARS C BIT.
    ADC #$02*NUMLEV+<3*ADDPRC>+13    ;MAKE SURE 2*NUMLEV+13 LOCS
...
*
IFE    REALIO-4,<
INLIN:    LDX #$80        ;NO PROMPT CHARACTER
    STX    CQPRMP
    JSR    CQINLN        ;GET A LINE ONTO PAGE 2
    CPXI    BUFLEN-1
    BCS    GDBUFS        ;NOT TOO MANY CHARACTERS
    LDX #BUFLEN-1
GDBUFS: LDA #$00        ;PUT A ZERO AT THE END
    STA    BUF,X
    TXA
    BEQ    NOCHR
LOPBHT: LDA    BUF-1,X
...
IFN REALIO-3,<
    CMP #$07        ;IS IT BOB ALBRECHT RINGING THE BELL
                ;FOR SCHOOL KIDS?
    BEQ    GOODCH>
    CMP #$0D        ;CARRIAGE RETURN?
    BEQ    FININ1        ;YES, FINISH UP.
IFN    REALIO-3,<
    CMP #$20        ;CHECK FOR FUNNY CHARACTERS.
    BCC    INLINC
    CMP #$7D        ;IS IT TILDA OR DELETE?
...
* APPEAR IN STMDSP.
*
BUFOFS    EQU    0            ;THE AMOUNT TO OFFSET THE LOW BYTE
...
KLOOP1: CMP #"0"        ;SKIP NUMERICS.
    BCC    MUSTCR
    CMP #$3C        ;":" AND ";" ARE ENTERED STRAIGHTAWAY.
    BCC    STUFFH
MUSTCR: STY    BUFPTR        ;SAVE BUFFER POINTER.
    LDY #$00        ;LOAD RESLST POINTER.
...
SAVE:    TSX            ;SAVE STACK POINTER
    ...
    LDA #$FE        ;MAKE ID BYTE EQUAL TO FF HEX
...
DATAN:    LDX #":"        ;"DATA" TERMINATES ON ":" AND NULL.
    SKIP2
REMN:    LDX #$00        ;THE ONLY TERMINATOR IS NULL.
    STX    CHARAC        ;PRESERVE IT.
    LDY #$00        ;THIS MAKES CHARAC=0 AFTER SWAP.
    STY    ENDCHR
EXCHQT: LDA    ENDCHR
    LDX    CHARAC
    STA    CHARAC
...
MORLIN: BCS    ONGRTS        ;IT IS NOT A DIGIT.
    ...
    LDA    LINNUM+1
    STA    INDEX
    CMP #$19        ;LINE NUMBER WILL BE .LT. 64000?
    BCS    SNERR3
    LDA    LINNUM
...
GETSPT: LDY #$02        ;GET PNTR TO DESCRIPTOR.
    LDADY    FACMO
...
FININL: LDA #0
    ...
    LDXYI    BUF-1>
IFE    BUFPAG,<
FININL: LDY #$00        ;PUT A ZERO AT END OF BUF.
    STY    BUF,X
...
CRDO:
IFE    EXTIO,<
    LDA #$0D        ;MAKE TRMPOS LESS THAN LINE LENGTH.
    STA    TRMPOS>
IFN    EXTIO,<
IFN    REALIO-3,<
    LDA    CHANNL
    BNE    GOCR
    STA    TRMPOS>
GOCR:    LDA #$0D>        ;X AND Y MUST BE PRESERVED.
    JSR    OUTDO
    LDA #10
    JSR    OUTDO
CRFIN:
...
MORCO1: SBC #CLMWID        ;GET [A] MODULUS CLMWID.
    BCS    MORCO1
    EOR #$FF        ;FILL PRINT POS OUT TO EVEN CLMWID SO
    ADC #1
...
IFE    REALIO-3,<
    ...
    LDA #" "
    SKIP2
CRTSKP: LDA #$1D>        ;COMMODORE'S SKIP CHARACTER.
    SKIP2
OUTQST: LDA #"?"
OUTDO:    IFN    REALIO,<
...
IFN    REALIO-3,<
    PHA
    CMP #$20        ;IS THIS A PRINTING CHAR?
...
IORELE: JSR    CQCCHN
    LDX #$00        ;RESET CHANNEL TO TERMINAL.
    STX    CHANNL
    RTS>
INPUT:    IFN    REALIO,<
...
READ:    LDXY    DATPTR        ;GET LAST DATA LOCATION.
    XWD    $200,$A9    ;LDA # TYA TO MAKE IT NONZERO.
IFE    BUFPAG,<
INPCON: >
    TYA
IFN    BUFPAG,<
    SKIP2
INPCON: LDA #$00>        ;SET FLAG THAT THIS IS INPUT
...
IFN    GETCMD,<
    ...
    INX
    STX    TXTPTR
    LDA #$00        ;ZERO TERMINATORS.
    STA    CHARAC
    BEQA    RESETC>
SETQUT: STA    CHARAC        ;ASSUME QUOTED STRING.
    CMP #$22        ;TERMINATORS OK?
    BEQ    NOWGET        ;YES.
...
NOWGET: STA    ENDCHR
    LDWD    TXTPTR
    ADC #$00        ;C IS SET PROPERLY ABOVE.
    BCC    NOWGE1
    INY
...
*
NEXT:    BNE    GETFOR
    LDY #$00        ;WITHOUT ARG CALL "FNDFOR" WITH
...
HAVFOR: TXS            ;SETUP STACK. CHOP FIRST.
    TXA
    CLC
    ADC #$04        ;POINT TO INCREMENT
    PHA            ;SAVE THIS POINTER TO RESTORE TO [A]
    ADC #$05+ADDPRC    ;POINT TO UPPER LIMIT
    STA    INDEX2        ;SAVE AS INDEX
    PLA            ;RESTORE POINTER TO INCREMENT
    LDY #$01        ;SET HI ADDR OF THING TO MOVE.
...
LOOPDN: TXA
    ADC #$02*ADDPRC+15        ;ADDS 16 WITH CARRY.
    TAX
    TXS            ;NEW STACK PNTR.
    JSR    CHRGOT
    CMP #$2C        ;COMMA AT END?
...
FRMEVL: LDX    TXTPTR
    ...
    DEC    TXTPTR+1
FRMEV1: DEC    TXTPTR
    LDX #$00        ;INITIAL DUMMY PRECEDENCE IS 0.
    SKIP1
...
LOPREL: SEC            ;PREP TO SUBTRACT.
    ...
    CMP #$01        ;RESET CARRY FOR ZERO ONLY.
...
QCHNUM: CMP #$64        ;RELATIONAL OPERATOR?
...
OROP:    LDY #$FF        ;MUST ALWAYS COMPLEMENT..
    SKIP2
ANDOP:    LDY #0
    STY    COUNT        ;OPERATOR.
...
STASGN: STA    FACSGN        ;KEEP FOR LATER.
    LDY #$FF        ;SET POINTER TO FIRST STRING. (ARG.)
...
GETCMP: LDADY    ARGMO        ;GET NEXT CHAR TO COMPARE.
    ...
    LDX #$01        ;SET A NEGATIVE DIFFERENCE.
...
INTERR: JMP    SNERR
PTRGT3: LDX #$00        ;ASSUME NO SECOND CHARACTER.
...
NOTSTR: CMP #"%"        ;INTEGER VARIABLE?
    ...
    STA    VARNAM>
TURNON: TXA
    ORA #$80        ;TURN ON MSB OF SECOND CHARACTER.
    TAX
...
NOTIT:    CLC
    LDA    LOWTR
    ADC #$06+ADDPRC    ;MAKES NO DIF AMONG TYPES.
    BCC    LOPFND
    INX
...
NOTEVE: STWD    HIGHDS        ;PLACE TO STUFF IT.
    ...
    LDY #$00        ;GET ADDR OF VARIABLE ENTRY.
    LDA    VARNAM
    STADY    LOWTR
    INY
    LDA    VARNAM+1
...
FMAPTR: LDA    COUNT
    ASL    A,
    ADC #$05        ;POINT TO ENTRIES. C CLR'D BY ASL.
    ADC    LOWTR
    LDY    LOWTR+1
    BCC    JSRGM
    INY
...
IFN    INTPRC,<
    ...
    LDY #$00        ;SET NUMBER OF DIMENSIONS TO ZERO.
...
GREASE: JSR    REASON        ;GET ROOM.
    ...
    LDA #$00        ;STORING [ACCA] IS FASTER THAN CLEAR.
    INC    CURTOL+1
    LDY    CURTOL
    BEQ    DECCUR
ZERITA: DEY
...
IFE    ADDPRC,<
    LDX #$04>
IFN    ADDPRC,<
    LDX #$05        ;THIS CODE ONLY WORKS FOR INTPRC=1
...
* BY [STRNG2].
*
STRLIT: LDX #$22        ;ASSUME STRING ENDS ON QUOTE.
    STX    CHARAC
    STX    ENDCHR
STRLT2: STWD    STRNG1        ;SAVE POINTER TO STRING.
    STWD    DSCTMP+1    ;IN CASE NO STRCPY.
    LDY #$FF        ;INITIALIZE CHARACTER COUNT.
STRGET: INY
...
IFN    ADDPRC,<
    ...
    ASL    A,
    ADC #$05        ;CARRY IS OFF AND OFF AFTER ADD.
    ADC    INDEX1
    STA    INDEX1
    BCC    ARYGET
    INC    INDEX1+1
...
MOVINS: LDY #$00        ;GET ADDR OF STRING.
    LDADY    STRNG1
    PHA
    INY
    LDADY    STRNG1
...
FRETMS: CPY    LASTPT+1    ;LAST ENTRY TO TEMP? 
    ...
    LDY #$00        ;ALSO CLEARS ZFLG SO WE DO REST OF FRETMP.
FRERTS: RTS            ;ALL DONE.
*
...
* # POSITION FOR #$02 CHARACTERS. IF #$02 GOES PAST END OF STRING
...
IFN    ADDPRC,<DC    I1'0'>
IFE    ADDPRC,<
LOGCN2: DC    I1'@2'        ; DEGREE-1
    DC    I1'@200'    ; 0.59897437
    DC    I1'@031'
    DC    I1'@126'
    DC    I1'@142'
    DC    I1'@200'    ; 0.96147080
    DC    I1'@166'
    DC    I1'@042'
    DC    I1'@363'
    DC    I1'@202'    ; 2.88539129
    DC    I1'@070'
    DC    I1'@252'
    DC    I1'100'>

IFN    ADDPRC,<
LOGCN2: DC    I1'@3'        ;DEGREE-1
    DC    I1'@177'    ;.43425594188
    DC    I1'@136'
    DC    I1'@126'
    DC    I1'@313'
    DC    I1'@171'
    DC    I1'@200'    ; .57658454134
    DC    I1'@023'
    DC    I1'@233'
    DC    I1'@013'
    DC    I1'@144'
    DC    I1'@200'    ; .96180075921
    DC    I1'@166'
    DC    I1'@070'
    DC    I1'@223'
    DC    I1'@026'
    DC    I1'@202'    ; 2.8853900728
    DC    I1'@070'
    DC    I1'@252'
    DC    I1'@073'
    DC    I1'@040'>
...
IFN    ADDPRC,<
    ...
    TYA
    JMP    SHFARG
LD100:    LDA #@100        ;ONLY WANT TWO MORE BITS.
...
FOUT8:    LDY #@0
FOUTIM: LDX #@200        ;FIRST PASS THRU, ACCX HAS MSB SET.
FOUT2:    LDA    FACLO
    CLC
...
IFE    ROMSW,<
    DS    1>        ;SO ZEROING AT TXTTAB DOESN'T PREVENT
                ;RESTARTING INIT
INITAT: INC    CHRGET+7    ;INCREMENT THE WHOLE TXTPTR.
    BNE    CHZGOT
    INC    CHRGET+8
CHZGOT: LDA    $EA60        ;A LOAD WITH AN EXT ADDR.
    CMP #":"        ;IS IT A ":"?
    BCS    CHZRTS        ;IT IS .GE. ":"
    CMP #" "        ;SKIP SPACES.
    BEQ    INITAT
...
INIT:
IFN    REALIO-3,<
    LDX #$FF        ;MAKE IT LOOK DIRECT IN CASE OF
...
IFE    REALIO,<HRLI 1,$200>    ;MAKE AN INST.
IFN    REALIO-3,<
    STA    START
    STA    RDYJSR>
    STA    JMPER
...
IFN    REALIO-3,<
    ...
    0
LASTWR::
    DS    100        ;SPACE FOR TEMP STACK.
IFE REALIO,<
TSTACK::DS    13600>

IF2,<
    PURGE    A,X,Y>
IFNDEF    START,<START    EQU    0>
    END    $Z+START